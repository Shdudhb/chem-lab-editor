...
test('flask catalog models are distinct and filter flask exposes a side port', () => {
  ...
  const flaskClamp = getEquipmentById('flask-clamp');
  assert.match(flaskClamp.svg, /M8 54h56v12H8/);
  assert.match(flaskClamp.svg, /M70 56C78 38 92 30 108 34l-4 12/);
  assert.match(flaskClamp.svg, /M88 38c7-4 13-5 20-4M88 82c7 4 13 5 20 4/);
  assert.doesNotMatch(flaskClamp.svg, /M15 60h33|M73 60l30-27|M47 43c-10 9/);
  assert.notEqual(flaskClamp.svg, universalClamp.svg);

  const flaskClampPoints = getSnapPoints({
    sourceId: 'flask-clamp', x: 0, y: 0, width: 120, height: 120,
  });
  assert.deepEqual(flaskClampPoints.map((point) => point.role), ['left', 'right']);
  ...
  const bunsenBurner = getEquipmentById('bunsen-burner');
  assert.match(bunsenBurner.svg, /M60 34C45 27 49 15 58 2c1 8 7 10 10 16/);
  assert.match(bunsenBurner.svg, /M50 34v50c0 5 4 8 10 8s10-3 10-8V34/);
  assert.match(bunsenBurner.svg, /x="45" y="62" width="30" height="18" rx="5"/);
  assert.match(bunsenBurner.svg, /M75 72h14c5 0 8 3 8 8v3h10/);
  assert.match(bunsenBurner.svg, /M22 101c6-10 19-15 38-15s32 5 38 15v7H22z/);
  assert.doesNotMatch(bunsenBurner.svg, /M34 91h52l10 13H24|M52 91V39h16v52|M68 66h19/);

  const bunsenPoints = getSnapPoints({
    sourceId: 'bunsen-burner', x: 0, y: 0, width: 120, height: 120,
  });
  ...
  const woodenStopper = getEquipmentById('wooden-stopper');
  assert.match(woodenStopper.svg, /M31 36c0-7 13-11 29-11s29 4 29 11L79 97/);
  assert.match(woodenStopper.svg, /<ellipse cx="60" cy="36" rx="29" ry="11"/);
  assert.match(woodenStopper.svg, /M43 55l8 2M64 49l9 3M48 73l7 2/);
  assert.equal((woodenStopper.svg.match(/<circle /g) || []).length, 3);
  assert.doesNotMatch(woodenStopper.svg, /M34 38h52l-8 60H42|m39 50 42 8|M42 98h36/);
  assert.notEqual(woodenStopper.svg, getEquipmentById('rubber-stopper').svg);

  const woodenStopperPoints = getSnapPoints({
    sourceId: 'wooden-stopper', x: 0, y: 0, width: 120, height: 120,
  });
...
test('catalog equipment has unique models for visually different apparatus', () => {
  ...
  const waterDeliveryTube = getEquipmentById('water-delivery-tube');
  assert.match(waterDeliveryTube.svg, /stroke="#78b9c8" stroke-width="9" d="M15 35C35 30 34 88 61 89s23-51 44-46"/);
  ...
  const testTubeBrush = getEquipmentById('test-tube-brush');
  assert.match(testTubeBrush.svg, /m75 21 20 16M65 26l28 23M58 34l30 24/);
  assert.match(testTubeBrush.svg, /M28 103 87 25M29 99c-5-4-11-5-14 0-4 7 6 14 13 4/);
  assert.doesNotMatch(testTubeBrush.svg, /m27 105 59-76|M78 28l14-14|M72 36l17 5/);
  assert.deepEqual(getSnapPoints({
  ...
  const ironRing = getEquipmentById('iron-ring');
  assert.match(ironRing.svg, /<ellipse cx="48" cy="54" rx="31" ry="18" stroke-width="7"/);
  assert.match(ironRing.svg, /<ellipse cx="48" cy="54" rx="31" ry="18" stroke="#667078" stroke-width="3"/);
  assert.match(ironRing.svg, /M96 47h10v14H96M106 54h8/);
  assert.doesNotMatch(ironRing.svg, /fill="#667078" fill-opacity="\.12"|cx="56" cy="52" rx="20"/);

  const ironRingPoints = getSnapPoints({
    sourceId: 'iron-ring', x: 0, y: 0, width: 120, height: 120,
  });
  ...
  const stirringRod = getEquipmentById('stirring-rod');
  assert.match(stirringRod.svg, /stroke-width="8" d="M25 100 95 20"/);
  assert.match(stirringRod.svg, /stroke="#f5fafb" stroke-width="4" d="M25 100 95 20"/);
  assert.doesNotMatch(stirringRod.svg, /m29 99 63-78|m25 103 8-8|M88 25l8-8/);
  assert.equal(stirringRod.supportsLiquid, false);
  assert.deepEqual(getSnapPoints({
